...
public class UserSolvedSyncStateJdbcRepository {
   ...
   //페이지 처리 단계에서 last_solved_count업데이트 x
    ...
    public Optional<SyncTarget> pickOneForObserve(LocalDateTime windowStart) {
        String sql = """
        SELECT s.user_id, u.boj_id
        FROM user_solved_sync_state s
        JOIN user u ON u.user_id = s.user_id
    ...
    """;
        var list = jdbcTemplate.query(sql, (rs, rowNum) ->
                new SyncTarget(rs.getLong("user_id"), rs.getString("boj_id")), windowStart);
        return list.stream().findFirst();
    }

    public Optional<DeltaPageTarget> pickOneForDeltaPage(LocalDateTime windowStart) {
        String sql = """
